...
#define initmodel initmodel__NMDA_SCHILLER
#define _net_receive _net_receive__NMDA_SCHILLER
#define _net_init _net_init__NMDA_SCHILLER
#define nrn_state_launcher nrn_state_NMDA_SCHILLER_launcher
#define nrn_cur_launcher nrn_cur_NMDA_SCHILLER_launcher
#define nrn_jacob_launcher nrn_jacob_NMDA_SCHILLER_launcher 
#if NET_RECEIVE_BUFFERING
#define _net_buf_receive _net_buf_receive_NMDA_SCHILLER
void _net_buf_receive(NrnThread*);
#endif
 
#define release release_NMDA_SCHILLER 
...
#define B _p[4*_STRIDE]
#define synon _p[5*_STRIDE]
#define DRon _p[6*_STRIDE]
#define DRoff _p[7*_STRIDE]
#define _v_unused _p[8*_STRIDE]
#define _g_unused _p[9*_STRIDE]
#define _tsav _p[10*_STRIDE]
...
#define mgblock mgblock_NMDA_SCHILLER
 ...
 inline double _f_mgblock( _threadargsprotocomma_ double );
 #pragma acc routine seq
 inline double mgblock( _threadargsprotocomma_ double );
 
static void _check_mgblock(_threadargsproto_); 
...
#define _gth 0
 static double Alpha_NMDA_SCHILLER = 0.94;
 static double Beta_NMDA_SCHILLER = 0.18;
 static double Cdur_NMDA_SCHILLER = 0.3;
 static double Erev_NMDA_SCHILLER = 0;
#define Rtau_NMDA_SCHILLER _thread1data[0]
#define Rtau _thread[_gth]._pval[0]
#define Rinf_NMDA_SCHILLER _thread1data[1]
#define Rinf _thread[_gth]._pval[1]
 static double mg_NMDA_SCHILLER = 1;
 static double usetable_NMDA_SCHILLER = 1;
...
#define _ppsize 3
 ...
 static double *_t_mgblock = nullptr;
...
static void _update_global_variables(NrnThread *_nt, Memb_list *_ml) {
   ...
   _global_variables->Alpha = Alpha_NMDA_SCHILLER;
   _global_variables->Beta = Beta_NMDA_SCHILLER;
   _global_variables->Cdur = Cdur_NMDA_SCHILLER;
   _global_variables->Erev = Erev_NMDA_SCHILLER;
   _global_variables->mg = mg_NMDA_SCHILLER;
   _global_variables->usetable = usetable_NMDA_SCHILLER;
...
void _net_buf_receive(NrnThread* _nt) {
  ...
  _nrb->_displ_cnt = 0;
  _nrb->_cnt = 0;
  /*printf("_net_buf_receive__NMDA_SCHILLER  %d\n", _nt->_id);*/
 ...
 static double _mfac_mgblock, _tmin_mgblock;
  ...
  if (_maktable) { double _x, _dx; _maktable=0;
   ...
   for (_i=0, _x=_tmin_mgblock; _i < 1001; _x += _dx, _i++) {
    _t_mgblock[_i] = _f_mgblock(_threadargs_, _x);
   }
   _sav_mg = mg;
  }
 }

 double mgblock(_threadargsproto_, double _lv) { 
#if 0
_check_mgblock(_threadargs_);
#endif
 return _n_mgblock(_threadargs_, _lv);
 }

 static double _n_mgblock(_threadargsproto_, double _lv){ int _i, _j;
 double _xi, _theta;
 if (!usetable) {
 return _f_mgblock(_threadargs_, _lv); 
}
...
static double _hoc_mgblock(void* _vptr) {
 ...
 _check_mgblock(_threadargs_);
#endif
 _r =  mgblock ( _threadargs_, *getarg(1) );
 return(_r);
}
...
static void _initlists(Memb_list *_ml){
 ...
 _slist1[1] = &(Roff) - _p;  _dlist1[1] = &(DRoff) - _p;
   if (!_t_mgblock) { _t_mgblock = makevector(1001*sizeof(double)); }
 }

...
def read_voltage_traces_from_csv(prefix, fname):
    ...
    # TODO: there is room for improvement here, as this is a bit hacky.
    # TODO at this point, the sim trial indices are already known at the db level. it's annoying to match them to corresponding vt files though
    # TODO: Which is why they're being inferred again here.
    dir_path = os.path.dirname(full_fname)
    syn_files = [os.path.basename(f) for f in glob.glob(os.path.join(dir_path, "*synapses*"))]
    cell_files = [os.path.basename(f) for f in glob.glob(os.path.join(dir_path, "*presynaptic_cells*"))]

    if not len(syn_files) == n_traces == len(cell_files):
        raise ValueError("Amount of runs does not match: {} synapse activation files, {} voltage traces, and {} cell activation files".format(len(syn_files), n_traces, len(cell_files)))
    
    # Special case: if the simulations runs are not consecutive. Fetch the run id form the synapse activation files instead
    INDICES = sorted(int(f.split("_")[1][3:]) for f in syn_files)
    index = [

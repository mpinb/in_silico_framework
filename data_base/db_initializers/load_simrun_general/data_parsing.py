...
def read_voltage_traces_from_files_pandas(prefix, fnames, meta=None):
    ...
    in :func:`~data_base.db_initializers.load_simrun_general.read_voltage_traces_by_filenames`.
    Each singular file is read using :func:`~data_base.db_initializers.load_simrun_general.read_voltage_traces_from_file`.
    ...
    See also:
        :func:`~data_base.db_initializers.load_simrun_general.read_voltage_traces_by_filenames`.
    """
...
def read_voltage_traces_from_file(prefix, fname, meta=None):
    ...
    See also:
        :func:`~data_base.db_initializers.load_simrun_general.read_voltage_traces_from_csv` and
        :func:`~data_base.db_initializers.load_simrun_general.read_voltage_traces_from_npz`
    """
...
def read_voltage_traces_from_csv(prefix, fname):
    ...
    Attention:
        This function assumes that the data in :paramprefix: is complete, e.g. no missing files.
        It infers the simulation trial index from the filename of the synapse activation files.
        :func:`_check_if_results_are_corrupted` checks if this is the case.
        This is automatically done in the :mod:`data_base.db_initializers.load_simrun_general` module.
...
# def read_voltage_traces_from_csv(prefix, fname):
#     """Reads a single :ref:`voltage_traces_csv_format` file as generated by the :mod:`simrun` package.
...
#     See also:
#         :func:`~data_base.db_initializers.load_simrun_general.read_voltage_traces_from_file`
...
def load_dendritic_voltage_traces(
    ...
    to each recsite label in :param:`suffix_key_dict`.
    ...
    Args:
        ...
        recsite_labels (List[str]):
            List of recording site labels, ordered the same way as :attr:`filelist`. Used to merge voltage traces fromt he same recording site together.
        repartition (bool):
            If True, the dask dataframe is repartitioned to 5000 partitions (only if it contains over :math:`10000` entries).

    Returns:
        Dict[str, :class:`dask.dataframe.DataFrame`]: 
...
def _read_n_vtraces(fname):
    ...
    Returns:
        int: Amount of voltage traces in :param:`fname`.
    """
...
def _estimate_average_vt_length(fnames, sample_size=1):
    """Estimate the length of all voltage traces in :param:`fnames`
    
    Reads the headers of :param:`sample_size` random files in :param:`fnames` to fetch
    ...
    Args:
        ...
        sample_size (int): Amount of voltage traces headers to read in order to infer their length.
            Set to ``1`` if you expect all votlage trace filenames in :param:`fnames` to be of equal length.
...
def _estimate_n_chunks(filelist, estimate_sample_size=1, partition_size=None):
    ...
    :attr:`~data_base.db_initializers.load_simrun_general.config.DASK_TARGET_PARTITION_SIZE`

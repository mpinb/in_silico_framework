...
def build_network_param_from_template(
    ...
    This function adds the following info:
    
    - Network embedding info (:ref:`syn_file_format`, :ref:`con_file_format` files and amount of connected cells per cell type)
    - Activity data
    ...
    See also:
        :func:`~build_network_param_from_template` for a version that also adds activity data beyond ongoing activity.
...
def build_ongoing_network_param_from_template(
    template_fn: str | Path | None = None,
    cell_nr_fn = None, # unused, but we keep for backwards compatibility
    syn_fn: str | Path = None,
    con_fn: str | Path = None,
